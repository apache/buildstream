...
class Mount():
    ...
    def _umount(self, path, stdout=sys.stdout, stderr=sys.stderr):
        ...
        cmd = self.platform.switch(linux=[utils.get_host_tool('umount'), '-R', path],
                                   default=[utils.get_host_tool('umount'), path])
    ...
    # exist and can be dangerous, requiring careful cleanup on
    # different platforms. It is recommended to use this function over
    # a manual mount invocation.
    #
    @contextmanager
    def bind_mount(self, dest, src=None, stdout=sys.stdout, stderr=sys.stderr,
        ...
        # Do NOT rbind since that causes /proc's sub-mounts to be
        # unmounted later
        options += self.platform.switch(linux=(',' if options else '') + 'rbind',
                                        default='')

        mount_type = self.platform.switch(sunos='lofs', default=None)

        path = self._mount(dest, src, mount_type, stdout, stderr, options)

        # On linux, make the rbind a slave to avoid unmounting vital
        # devices in /proc
        if self.platform.platform_name.startswith('linux'):
...
class SandboxChroot(Sandbox):
    ...
    def mount_dirs(self, flags, stdout, stderr):
        ...
        if flags & SandboxFlags.INTERACTIVE:
            ...
            # Solaris also requires /devices to be mounted
            if self.platform.platform_name.startswith('sunos'):
                devices_src, devices_point = self.get_mount_location('/devices/', 'devices')
        ...
        with ExitStack() as stack:
            ...
            if flags & SandboxFlags.INTERACTIVE:
                ...
                if self.platform.platform_name.startswith('sunos'):
                    stack.enter_context(self.mount.bind_mount(devices_point, devices_src, stdout, stderr))

            # AIX does not support mounting a proc type FS
            if self.platform.platform_name.startswith('aix'):
                stack.enter_context(self.mount.bind_mount(proc_point, proc_src, stdout, stderr, ro=True))
            else:
