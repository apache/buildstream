...
def test_buildtree_checkout_fail(cli, datafiles):
    ...
    tar = os.path.join(cli.directory, "buildtree-checkout.tar")

    res = cli.run(project=project, args=["buildtree", "checkout", "--directory", checkout, element_name])
    res.assert_main_error(ErrorDomain.STREAM, "missing-buildtree-and-buildroot")

    res = cli.run(
        project=project, args=["buildtree", "checkout", "--buildroot", "--directory", checkout, element_name]
    )
    res.assert_main_error(ErrorDomain.STREAM, "missing-buildroot")
    ...
    res = cli.run(
        project=project, args=["buildtree", "checkout", "--compression", "gz", "--directory", checkout, element_name]
    )
...
def test_buildtree_checkout(cli, datafiles):
    ...
    for compression in ["gz", "xz", "bz2"]:
        res = cli.run(
            project=project,
            args=["buildtree", "checkout", "--tar", "-", "--compression", compression, element_name],
            binary_capture=True,
        )
        res.assert_success()
    ...
    res = cli.run(
        project=project,
        args=["buildtree", "checkout", "--buildroot", "--hardlinks", "--directory", checkout, element_name],
    )
    res.assert_success()
...
#
# Verify checkout buildtree on non buildable element
#
@pytest.mark.datafiles(DATA_DIR)
@pytest.mark.skipif(not HAVE_SANDBOX, reason="Only available with a functioning sandbox")
def test_buildtree_checkout_non_buildable(cli, datafiles):
    project = str(datafiles)
    ...
    # verify checkout buildtree only
    res = cli.run(project=project, args=["buildtree", "checkout", "--directory", checkout, element_name])
    res.assert_main_error(ErrorDomain.STREAM, "missing-buildtree-buildroot-exists")
