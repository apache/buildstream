...
#
# Note that while staging order is deterministic, build planning is not entirely deterministic,
# so we make 2 failing builds occur and assert that only one of them failed and the other one
# didn't try to build.
#
@pytest.mark.datafiles(DATA_DIR)
@pytest.mark.skipif(not HAVE_SANDBOX, reason="Only available with a functioning sandbox")
