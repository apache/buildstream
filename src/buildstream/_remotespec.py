...
class RemoteSpec:
    ...
    # minimum throughput in bytes per second to extend the grpc request timeout
    #
    @property
    def min_throughput(self) -> Optional[int]:
        if self._connection_config:
            return self._connection_config.get_int("min-throughput", None)
        return None
    ...
    def to_localcas_remote(self, remote):
        ...
        if self.min_throughput is not None:
            remote.min_throughput = self.min_throughput
