...
class Element(Plugin):
    ...
    # _get_artifact_cas_digest():
    #
    # Returns the artifact CAS digest
    #
    # Returns:
    #    (str|None): The artifact CAS digest for this Element, or None
    #
    # None is returned if no CAS artifact is available for this element.
    #
    def _get_artifact_cas_digest(self):
        if self.__artifact is None:
            return None
        if not self.__artifact.query_cache():
            return None
        return self.__artifact.get_files().get_artifact_cas_digest()
