...
class SandboxConfig:
    def __init__(
        self, *, build_os: str, build_arch: str, build_uid: Optional[int] = None, build_gid: Optional[int] = None, remote_apis_socket_path: Optional[str] = None, remote_apis_socket_action_cache_update_enabled: bool = False
    ):
        ...
        self.remote_apis_socket_action_cache_update_enabled = remote_apis_socket_action_cache_update_enabled
    ...
    def to_dict(self) -> Dict[str, Union[str, int, bool]]:
        ...
        if self.remote_apis_socket_path is not None:
            ...
            if self.remote_apis_socket_action_cache_update_enabled:
                sandbox_dict["remote-apis-socket-action-cache-update-enabled"] = True
    ...
    def new_from_node(cls, config: "MappingNode[Node]", *, platform: Optional[Platform] = None) -> "SandboxConfig":
        ...
        if remote_apis_socket:
            remote_apis_socket.validate_keys(["path", "action-cache-update-enabled"])
            remote_apis_socket_path = remote_apis_socket.get_str("path")
            remote_apis_socket_action_cache_update_enabled = remote_apis_socket.get_bool("action-cache-update-enabled", default=False)
        else:
            remote_apis_socket_path = None
            remote_apis_socket_action_cache_update_enabled = False

        return cls(build_os=build_os, build_arch=build_arch, build_uid=build_uid, build_gid=build_gid, remote_apis_socket_path=remote_apis_socket_path, remote_apis_socket_action_cache_update_enabled=remote_apis_socket_action_cache_update_enabled)
