...
@click.argument('directory', nargs=1, required=False,
                type=click.Path(readable=True, writable=True, resolve_path=True))
@click.pass_obj
def init(app, project_name, format_version, element_path, force, directory):
    ...
    interactive session.
    """
    app.init_project(project_name, format_version, element_path, force, directory)
